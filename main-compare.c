...
int main(int argc, char **argv)
{
	...
	for (int i = 1; i < argc; ++i)
	{
		...
		else
		{
			...
			KosinskiDecompress(in_file_buffer, &decompress_callbacks,
			#ifdef DEBUG
				true
			#else
				false
			...
			KosinskiCompress(MemoryStream_GetBuffer(&uncompressed_buffer), MemoryStream_GetPosition(&uncompressed_buffer), &compress_callbacks,
			#ifdef DEBUG
				true
			#else
				false
